 ...
 try{
  ...
  for(let round=0;round<4;round++){
   ...
   if(m.tool_calls?.length){
    ...
    for(const call of parsed){calls++;const r=await executeTool(ctx,call.function.name,JSON.parse(call.function.arguments),id,signal);used.push(call.function.name);if(r.proposal)proposals.push(r.proposal);messages.push({role:'tool',tool_call_id:call.id,content:JSON.stringify({trust:'UNTRUSTED_DATA',data:r.data})});}
   }else{
