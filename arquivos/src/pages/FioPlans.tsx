...
export function FioPlans(p:WorkspaceProps){
 ...
 async function manageCharge(action:'cancel_pending'|'resend'){
  ...
  try{const result=await api<{subscription:BillingState|null}>('/saas/charge',p.data.shop.id,{action,confirmed:true});setBilling(result.subscription);await p.refresh();if(action==='cancel_pending'){setCheckoutPlan(null);p.notify('Contratação pendente cancelada. Você já pode escolher outro plano.');}else p.notify('Cobrança consultada. Use somente o Pix válido exibido.');}catch(e){setCheckoutError(checkoutMessage(e));}finally{setBusy(false);}
 }
 ...
 return <>
  ...
  {billing?.providerStatus==='pending_first_payment'&&<p className="muted">Esta contratação ainda não foi paga. O teste grátis não gera cobranças sozinho. Para escolher outro plano, abra Ver cobrança e cancele a contratação pendente.</p>}
