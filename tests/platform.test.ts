...
describe('API PLATFORM_ADMIN',()=>{
 ...
 it('rewrite da Vercel mantém API fora do SPA e manifest usa ícones existentes',()=>{const v=JSON.parse(readFileSync('vercel.json','utf8'));expect(v.rewrites).toContainEqual({source:'/platform/:path*',destination:'/index.html'});expect(v.rewrites.some((r:{source:string})=>r.source==='/(.*)'||r.source.startsWith('/api'))).toBe(false);const m=JSON.parse(readFileSync('public/manifest-platform.webmanifest','utf8'));expect(m.start_url).toBe('/platform');expect(m.display).toBe('standalone');for(const icon of m.icons)expect(readFileSync(resolve('public','.'+icon.src)).length).toBeGreaterThan(0);});
});
