...
export function AssistantChat({role,plan,aiEnabled,shopId,demo,base}:{role:Role;plan:Plan;aiEnabled:boolean;shopId:string;demo:boolean;base:string}) {
 const [messages,setMessages]=useState<Message[]>([]),[value,setValue]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState<RequestError|null>(null),[conversationId,setConversationId]=useState<string>(),[history,setHistory]=useState<{id:string;title:string}[]|null>(null);
 ...
 async function submit(e:FormEvent){e.preventDefault();if(!value.trim()||busy)return;setError(null);
  ...
  const text=value.trim();setBusy(true);
  try{const result=await api<{conversationId:string;message:string}>('/assistant',shopId,{message:text,...(conversationId?{conversationId}:{})});setConversationId(result.conversationId);setMessages(prev=>[...prev,{id:crypto.randomUUID(),role:'user',content:text},{id:crypto.randomUUID(),role:'assistant',content:result.message}]);setValue('');}
  catch(e){setError(e instanceof RequestError?e:new RequestError('ERROR','Não foi possível responder.'));}finally{setBusy(false);}
 }
