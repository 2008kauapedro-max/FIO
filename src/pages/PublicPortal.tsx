...
export function PublicPortal(){
 ...
 return <div className="public-portal branded-portal" style={portalStyle}>
  ...
  <main className="public-content">
   {mayOfferInstall(navigator.userAgent,standalone)&&!installDismissed&&<section className="public-install-nudge"><div className="install-nudge-icon"><Download size={18}/></div><div><strong>Instale o app de {title}</strong><span>Abra sua agenda direto pela tela inicial.</span></div><button className="install-nudge-action brand-button" onClick={()=>void install()}>{installEvent?'Instalar':ios?'Como instalar':'Instalar app'}</button><button className="install-nudge-close" aria-label="Fechar" onClick={dismissInstall}><X size={16}/></button></section>}
