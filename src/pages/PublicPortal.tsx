...
type PublicData={shop:PublicShop;palette?:PublicPalette|null;services:{id:string;name:string;duration_minutes:number;price_cents:number}[];team:{user_id:string;display_name:string;role:'OWNER'|'BARBER'}[];subscriptionPlans:{id:string;name:string;cuts:number;validity_days:number;price_cents:number;active:boolean}[]};
...
export function PublicPortal(){
 ...
 return <div className="public-portal branded-portal" style={portalStyle}>
  ...
  <main className="public-content">
   ...
   {data.subscriptionPlans?.length>0&&<section><div className="section-title"><h2>Planos de assinatura</h2></div><div className="public-plan-grid">{data.subscriptionPlans.map(plan=><article key={plan.id} className="public-plan-card"><strong>{plan.name}</strong><b>{money(plan.price_cents)}</b><span>{plan.cuts} corte{plan.cuts===1?'':'s'} · {plan.validity_days} dias</span>{whatsappUrl(data.shop.whatsapp)&&<a className="brand-button" href={`${whatsappUrl(data.shop.whatsapp)}?text=${encodeURIComponent(`Olá! Quero assinar o plano ${plan.name} pelo FIO.`)}`} target="_blank" rel="noreferrer">Quero assinar</a>}</article>)}</div></section>}
