...
function PlatformWorkspace({admin}:{admin:PlatformAdmin}){
 ...
 const mobileMore=[['/assinaturas','Assinaturas',Wallet],['/atividade','Atividade',Activity],['/alertas','Alertas',Bell],['/configuracoes','Configurações',Settings]] as const;
...
function Shops(){
 ...
 const shopParams=new URLSearchParams({page:String(page)});if(term.trim())shopParams.set('search',term.trim());if(status!=='all')shopParams.set('status',status);
...
function ActivityList({shop}:{shop?:string}){
 ...
 const r=useResource<Page<AuditEvent>>(`/platform/activity?page=${page}${shop?'&shop='+shop:''}${query?'&'+query:''}`);
