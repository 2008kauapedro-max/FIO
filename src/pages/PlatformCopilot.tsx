...
export function PlatformAlerts(){
 const [severity,setSeverity]=useState(''),[status,setStatus]=useState('open'),[page,setPage]=useState(1),[version,setVersion]=useState(0),[data,setData]=useState<{items:Alert[];total:number}|null>(null),[error,setError]=useState(''),[loading,setLoading]=useState(true),[proposal,setProposal]=useState<Proposal|null>(null),[busy,setBusy]=useState(false);
 useEffect(()=>{let active=true;setLoading(true);setError('');api<{items:Alert[];total:number}>(`/platform/alerts?page=${page}&limit=10&status=${status}${severity?'&severity='+severity:''}`).then(r=>{if(active)setData(r);}).catch(e=>{if(active)setError(e.message);}).finally(()=>{if(active)setLoading(false);});return()=>{active=false;};},[severity,status,page,version]);
 ...
 return <><header className="pf-heading"><div><h1>Alertas</h1><p>{data?`${data.total} ${status==='open'?'alertas abertos':'alertas resolvidos'}`:'Eventos operacionais da plataforma'}</p></div></header><div className="pf-filters"><label>Gravidade<select value={severity} onChange={e=>{setSeverity(e.target.value);setPage(1);}}><option value="">Todas</option><option value="critical">Crítica</option><option value="warning">Atenção</option><option value="info">Informativa</option></select></label><label>Status<select value={status} onChange={e=>{setStatus(e.target.value);setPage(1);}}><option value="open">Abertos</option><option value="resolved">Resolvidos</option></select></label><button onClick={()=>setVersion(v=>v+1)}>Atualizar</button></div>{error&&<p role="alert">{error}</p>}{loading?<p role="status">Carregando alertas…</p>:data?.items.length?data.items.map(a=><article className="pc-alert" key={a.id}><div><small>{a.severity==='critical'?'CRÍTICO':a.severity==='warning'?'ATENÇÃO':'INFORMAÇÃO'} · {new Date(a.created_at).toLocaleString('pt-BR')}</small><strong>{a.title}</strong><details><summary>Ver detalhe</summary><p>{a.description}</p>{a.barbershop_id&&<small>Barbearia: {a.barbershop_id}</small>}</details></div>{a.status==='open'&&<button disabled={busy} onClick={()=>void resolve(a.id)}>Resolver</button>}</article>):<p className="pf-state">Nenhum alerta neste filtro.</p>}{proposal&&<ProposalCard key={proposal.id} proposal={proposal} done={()=>setVersion(v=>v+1)}/>}<div className="pf-pager"><button disabled={page===1||loading} onClick={()=>setPage(p=>p-1)}>Anterior</button><small>Página {page}</small><button disabled={loading||page*10>=(data?.total??0)} onClick={()=>setPage(p=>p+1)}>Próxima</button></div></>;
}
