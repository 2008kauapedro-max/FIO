...
import { ArrowUpRight,ArrowLeft,Plus,Scissors,Clock3,Users,CalendarDays,Check,Search,Copy,SlidersHorizontal,MessageCircle,Star,Link as LinkIcon,Download,X,Share2,SquarePlus,UserRound,Store,ShieldCheck,KeyRound,LogOut,Palette,Eye,EyeOff,Printer,Info,Crown } from 'lucide-react';
...
export function Subscriptions(p:WorkspaceProps){
 const owner=p.data.membership.role==='OWNER';
 const [planModal,setPlanModal]=useState(false),[assignModal,setAssignModal]=useState(false),[clientId,setClientId]=useState(p.data.customers[0]?.id??''),[planId,setPlanId]=useState(p.data.subscriptionPlans[0]?.id??''),[name,setName]=useState(''),[cuts,setCuts]=useState('4'),[days,setDays]=useState('30'),[price,setPrice]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState('');
 async function createPlan(e:FormEvent){e.preventDefault();setBusy(true);setError('');try{await api('/subscription-plans',p.data.shop.id,{name,cuts:Number(cuts),validityDays:Number(days),priceCents:Math.round(Number(price.replace(',','.'))*100)});await p.refresh();setPlanModal(false);setName('');setPrice('');p.notify('Plano publicado para os clientes.');}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
 ...
 return <><PageTitle eyebrow="PLANOS DA BARBEARIA" title={owner?'Assinaturas':'Minha assinatura'} description={owner?'Crie os planos que o cliente pode escolher e acompanhe quem está ativo.':'Veja sua assinatura e os planos disponíveis.'} action={owner?<div className="page-actions"><button className="secondary" onClick={()=>setAssignModal(true)}><UserRound size={17}/>Adicionar a cliente</button><button className="primary" onClick={()=>setPlanModal(true)}><Plus size={17}/>Novo plano</button></div>:undefined}/><div className="section-title"><h2>Planos disponíveis</h2><span className="muted">{p.data.subscriptionPlans.filter(x=>x.active).length} publicados</span></div>{p.data.subscriptionPlans.filter(x=>x.active).length?<div className="subscription-grid">{p.data.subscriptionPlans.filter(x=>x.active).map(plan=><article key={plan.id} className="subscription-card plan-catalog-card"><span className="eyebrow">PLANO</span><h2>{plan.name}</h2><strong>{money(plan.price_cents)}</strong><p>{plan.cuts} corte{plan.cuts===1?'':'s'} · {plan.validity_days} dias de validade</p>{!owner&&<a className="primary full whatsapp-button" target="_blank" rel="noreferrer" href={`${whats(p.data.shop.whatsapp)}?text=${encodeURIComponent(`Olá! Quero assinar o plano ${plan.name} pelo FIO.`)}`}><MessageCircle size={17}/>Quero assinar</a>}</article>)}</div>:<Empty title="Nenhum plano publicado">{owner?'Crie o primeiro plano para ele aparecer aos clientes.':'A barbearia ainda não publicou planos.'}</Empty>}<div className="section-title subscriptions-active-title"><h2>{owner?'Clientes com assinatura':'Sua assinatura ativa'}</h2><span className="muted">{active.length} ativa{active.length===1?'':'s'}</span></div>{active.length?<div className="subscription-grid">{active.map(s=><article key={s.id} className="subscription-card"><span className="eyebrow">ASSINATURA ATIVA</span><h2>{s.name}</h2>{owner&&s.client_id&&<p>{p.data.customers.find(c=>c.id===s.client_id)?.name??'Cliente'}</p>}<strong>{s.remaining_cuts}<span> cortes restantes</span></strong><p>Até {new Date(s.expires_at).toLocaleDateString('pt-BR',{timeZone:p.data.shop.timezone})}</p></article>)}</div>:<Empty title={owner?'Nenhum cliente com assinatura ativa':'Você não tem assinatura ativa'}/>} {owner&&planModal&&<Modal title="Novo plano para clientes" onClose={()=>setPlanModal(false)}><form onSubmit={createPlan}><Field label="Nome do plano"><input required minLength={2} maxLength={100} value={name} onChange={e=>setName(e.target.value)}/></Field><div className="form-grid"><Field label="Cortes"><input inputMode="numeric" required value={cuts} onChange={e=>setCuts(e.target.value.replace(/\D/g,''))}/></Field><Field label="Validade (dias)"><input inputMode="numeric" required value={days} onChange={e=>setDays(e.target.value.replace(/\D/g,''))}/></Field></div><Field label="Preço (R$)"><input inputMode="decimal" required value={price} onChange={e=>setPrice(e.target.value.replace(/[^0-9,.]/g,''))}/></Field>{error&&<p className="notice" role="alert">{error}</p>}<button className="primary full" disabled={busy}>Publicar plano</button></form></Modal>} {owner&&assignModal&&<Modal title="Adicionar assinatura ao cliente" onClose={()=>setAssignModal(false)}><form onSubmit={assign}><Field label="Cliente"><select required value={clientId} onChange={e=>setClientId(e.target.value)}>{p.data.customers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></Field><Field label="Plano"><select required value={planId} onChange={e=>setPlanId(e.target.value)}>{p.data.subscriptionPlans.filter(x=>x.active).map(plan=><option key={plan.id} value={plan.id}>{plan.name} · {money(plan.price_cents)}</option>)}</select></Field>{error&&<p className="notice" role="alert">{error}</p>}<button className="primary full" disabled={busy||!clientId||!planId}>Confirmar assinatura</button></form></Modal>}</>;
}
export function Reports(p:WorkspaceProps){
 const today=dayKey(new Date().toISOString(),p.data.shop.timezone),weekStart=(()=>{const d=new Date();const day=(d.getDay()+6)%7;d.setDate(d.getDate()-day);return dayKey(d.toISOString(),p.data.shop.timezone);})();
 const [selected,setSelected]=useState<Appointment|null>(null),[busy,setBusy]=useState(false),[from,setFrom]=useState(weekStart),[to,setTo]=useState(today),[details,setDetails]=useState(false);
 const inRange=(iso:string)=>{const k=dayKey(iso,p.data.shop.timezone);return k>=from&&k<=to;};
 const payments=p.data.payments.filter(x=>inRange(x.created_at));const total=payments.reduce((sum,x)=>sum+x.amount_cents,0);
 const completed=p.data.appointments.filter(a=>a.status==='completed'&&inRange(a.starts_at));
 async function receive(){if(!selected)return;setBusy(true);try{if(p.demo){p.notify('Demonstração: nenhum recebimento real foi registrado.');}else{await api('/payments',p.data.shop.id,{appointmentId:selected.id,confirmed:true});await p.refresh();p.notify('Recebimento registrado.');}setSelected(null);}catch(e){p.notify((e as Error).message);}finally{setBusy(false);}}
 const print=()=>window.print();
 return <><PageTitle eyebrow="RELATÓRIO" title="Financeiro" description="Recebimentos registrados, sem estimativas." action={<button className="secondary print-hide" onClick={print}><Printer size={17}/>Imprimir relatório</button>}/><div className="report-controls print-hide"><Field label="De"><input type="date" value={from} max={to} onChange={e=>setFrom(e.target.value)}/></Field><Field label="Até"><input type="date" value={to} min={from} max={today} onChange={e=>setTo(e.target.value)}/></Field></div><section className="report-total"><div className="report-total-head"><span className="eyebrow">RECEBIDO NO PERÍODO</span><button className="text-button print-hide" onClick={()=>setDetails(v=>!v)}><Info size={15}/>{details?'Ocultar detalhes':'Saiba mais'}</button></div><strong>{money(total)}</strong><p className="muted">{new Date(from+'T12:00:00').toLocaleDateString('pt-BR')} até {new Date(to+'T12:00:00').toLocaleDateString('pt-BR')}</p>{details&&<div className="report-details"><span><b>{payments.length}</b> recebimento{payments.length===1?'':'s'} confirmado{payments.length===1?'':'s'}</span><span><b>{completed.length}</b> atendimento{completed.length===1?'':'s'} concluído{completed.length===1?'':'s'}</span><span>Somente pagamentos registrados entram neste total.</span></div>}</section><div className="section-title"><h2>Atendimentos concluídos</h2><span className="muted">Período selecionado</span></div><p className="muted">Abra um atendimento para confirmar um recebimento ainda não registrado.</p>{completed.length?completed.map(a=><AppointmentRow key={a.id} data={p.data} appointment={a} onClick={()=>setSelected(a)}/>):<Empty title="Nenhum atendimento concluído neste período"/>}{selected&&<Modal title="Confirmar recebimento" onClose={()=>setSelected(null)}><p>Você recebeu {money(selected.price_cents)} por este atendimento?</p><p className="muted">Esta ação registra um valor recebido. Não cobra o cliente. Recebimentos duplicados são bloqueados.</p><button className="primary full" disabled={busy} onClick={receive}>Confirmar recebimento</button></Modal>}</>;
}
...
export function Settings(p:WorkspaceProps){
 ...
 const [section,setSection]=useState<'profile'|'barbershop'|'plan'|'access'|'account'>('profile');
 ...
 const links={gestao:`${origin}/acesso/gestao`,equipe:`${origin}/acesso/equipe`,clientes:`${origin}/barbearia/${p.data.shop.slug}`};
 ...
 async function uploadBrand(file:File|undefined,kind:'logo'|'cover'|'background'){
  if(!file||!supabase)return;if(!['image/jpeg','image/png','image/webp'].includes(file.type)||file.size>5*1024*1024){p.notify('Use JPG, PNG ou WebP de até 5 MB.');return;}
  setBusy(true);try{const user=(await supabase.auth.getUser()).data.user;if(!user)throw Error('Entre novamente para enviar a imagem.');const ext=file.type==='image/jpeg'?'jpg':file.type.split('/')[1];const path=`${p.data.shop.id}/${user.id}/settings-${kind}-${Date.now()}.${ext}`;const up=await supabase.storage.from('branding-assets').upload(path,file,{contentType:file.type,upsert:false});if(up.error)throw up.error;const url=supabase.storage.from('branding-assets').getPublicUrl(path).data.publicUrl;if(kind==='logo')setLogoUrl(url);if(kind==='cover')setCoverUrl(url);if(kind==='background')setBackgroundUrl(url);p.notify('Imagem pronta. Toque em Salvar identidade para publicar.');}catch(e){p.notify((e as Error).message||'Não foi possível enviar a imagem.');}finally{setBusy(false);}
 }
    ...
    {section==='barbershop'&&owner&&<>
     <section className="settings-card branding-card">
      ...
      <div className="branding-upload-grid">
       <label className="branding-upload"><span>Logo</span><small>{logoUrl?'Imagem selecionada':'Escolher da galeria'}</small><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>void uploadBrand(e.target.files?.[0],'logo')}/></label>
       <label className="branding-upload"><span>Capa</span><small>{coverUrl?'Imagem selecionada':'Escolher da galeria'}</small><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>void uploadBrand(e.target.files?.[0],'cover')}/></label>
       <label className="branding-upload"><span>Fundo</span><small>{backgroundUrl?'Imagem selecionada':'Escolher da galeria'}</small><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>void uploadBrand(e.target.files?.[0],'background')}/></label>
      </div>
