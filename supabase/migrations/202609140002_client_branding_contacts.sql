-- FIO: contato entre participantes e identidade visual do app do cliente.
...
alter table public.barbershops
  add constraint barbershops_accent_color_format
  check (accent_color ~ '^#[0-9A-Fa-f]{6}$');

-- Todo membro autenticado da mesma barbearia pode visualizar os contatos da equipe.
...
using (
  ...
  and (
    user_id = auth.uid()
    or role in ('OWNER','BARBER')
    or public.member_role(barbershop_id) = 'OWNER'
  )
);
...
begin
  ...
  update public.barbershops
     set public_title = nullif(trim(coalesce(p_title,'')),''),
